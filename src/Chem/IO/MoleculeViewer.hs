...
viewerHTML title payloadValue =
  unlines $
    ...
    , "      if (!point) return;"
    , "      const strength = Math.min(5, Math.abs(Number(atom.charge || 0)));"
    ...
    , "      const fieldRadius = ionic ? Math.max(120, point.radius * radiusFactor) : Math.max(54, point.radius * radiusFactor);"
    ...
    , "          const overlapping = laneIds.length > 1;"
    , "          const lane = Math.max(0, laneIds.indexOf(system.id));"
    , "          const laneOffset = overlapping ? (lane - (laneIds.length - 1) / 2) * 7 : 0;"
    , "          if (isIonicSystem(system)) {"
    , "            const gradient = chargeGradientForEdge(atomMap, edge);"
    , "            drawBondLines(projected.get(edge.a), projected.get(edge.b), { lineCount: 1, width: active ? 4 : 2.5, spacing: 5.2, colorA: gradient.colorA, colorB: gradient.colorB, alpha: active ? 0.95 : 0.22, dash: overlapping ? [7, 6] : null, offset: laneOffset });"
    , "            return;"
    , "          }"
    , "          const covalentLines = standardCovalentLineCount(system);"
    , "          if (covalentLines) {"
    , "            if (!selected && !overlapping) return;"
    , "            drawBondLines(projected.get(edge.a), projected.get(edge.b), { lineCount: covalentLines, width: 2.9, spacing: 5.6, color: covalentBondColor, alpha: selected ? 0.98 : 0.58, dash: overlapping ? [7, 6] : null, offset: laneOffset });"
    , "            return;"
    , "          }"
    ...
    , "    const visibleSystems = displaySystems(payload.systems);"
    ...
    , "    } else {"
    , "      visibleSystems.forEach(function (system) {"
    , "      const button = document.createElement('button');"
    , "      button.type = 'button';"
